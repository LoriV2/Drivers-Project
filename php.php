...
<form action="php.php" method="POST">
    ...
    <input type="text" id="search" name="seearch" placeholder="Nome da companhia" onfocus="focuss(this)" onblur="blurr(this)">
    <button onclick="TheResult()">Search</button>
</form>
...
<body>
    ...
    <table class="table table-hover">
        ...
        <tbody>
            ...
            $Categoria = $_POST['Categoriia'];
            $Nome = $_POST['seearch'];
            $stmt = $conn->prepare("SELECT * FROM companies_contacts WHERE Categoria = '%$Categoria%' AND  NOME = (?) ");
            if (!$Categoria == Null) {
                if (!$Nome == null) {
                    $stmt->bind_param("ss", $Categoria, $Nome);
                    $result = mysqli_query($conn, $stmt);
                    while ($row = mysqli_fetch_array($result)) {
                        echo "<tr>" . "<td>" . htmlspecialchars($row['NOME']) .
                            "<td>" . htmlspecialchars($row['RESPONS_VEL']) .
                            "<td>" . htmlspecialchars($row['TLM']) .
                            "<td>" . htmlspecialchars($row['MORADA']) .
                            "<td>" . htmlspecialchars($row['HOR_RIO']) .
                            "<td>" . htmlspecialchars($row['EMAIL']) .
                            "<td>" . htmlspecialchars($row['Restaurantes']) .
                            "<td>" . htmlspecialchars($row['Descri_o_da_empresa']) .
                            "<td>" . htmlspecialchars($row['Observa_es']) . "</td></tr>";
                    }
                    $stmt->close();
                    $conn->close();
                } else  {
